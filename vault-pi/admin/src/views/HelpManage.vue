      ...
      <div class="table-container">
        <table>
          ...
          <tbody>
            ...
            <tr v-for="item in list" :key="item.id" :class="{ 'row-top': item.isTop === '1' }">
              <td>{{ item.id }}</td>
              <td>
                <div class="sort-stack">
                  <span class="sort-val">Sort: {{ item.sort }}</span>
                  <span v-if="item.isTop === '1'" class="badge badge-warning">置顶</span>
                </div>
              </td>
              ...
              <td class="action-cell">
                ...
                <button class="btn-text" @click="toggleTop(item)">{{ item.isTop === '1' ? '取消置顶' : '置顶' }}</button>
...
.sort-val { font-size: 11px; color: var(--ui-muted); }
.class-tag { background: var(--table-th-bg); border: 1px solid var(--border-color); color: var(--text-secondary); padding: 2px 8px; border-radius: 4px; font-size: 12px; }

.col-title { font-weight: 500; color: var(--text-main); max-width: 300px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.lang-tag { background: #f0fff4; color: var(--color-emerald); padding: 2px 6px; border-radius: 4px; font-size: 11px; font-weight: 700; }
...
.modal-content { background: var(--ui-surface-2); border-radius: 12px; width: 850px; max-width: 95%; max-height: 90vh; display: flex; flex-direction: column; box-shadow: 0 20px 25px -5px rgba(0,0,0,0.1); }
